...
- 每个发布标签必须与应用版本一致（例如 `v1.1.0`）。推送该标签会自动执行格式、lint、类型、测试、前端构建和 Rust
...
- 项目 / 任务（两层父子、含「已推迟」、四级优先级）/ Milestone / 会议与行动项
...
- 月视图日历、全局文件页（仅 URL/路径）、全量 JSON 导入导出、CSV 单向导出、SQLite 备份恢复
...
- 周期会议在会议页创建和维护：选择每周或隔周、星期、起止日期、时间、项目（可选）和议程模板。日历中的周期预期项可按次物化、跳过或取消物化；批量物化默认最多未来 3 次且全量成功或回滚。
- v1.1 未引入项目/任务状态机；「已推迟」可与任意状态自由切换，且不进入逾期与提醒。
...
- SQLite 迁移 0001（8 张基础表、索引、触发器、外键级联）与迁移 0002（任务生命周期列、索引、层级触发器）、
  ...
  0005（结构化风险表）、0006（项目链接可选备注）、0007（人员与独立参与关系）及 0008
  （人员可选字段与已推迟 CHECK 扩展）；0002–0007 保持纯增量，0008 仅为 CHECK 扩展重建项目/任务表；启动时断言
...
- **日历**：`/calendar` 月视图，六周网格、周一为首日，同日的任务截止 / 会议 / 里程碑以中文文字标签
  ...
  跨日任务以横向色条跨越日期；实线、虚线、双边框和红色边框图例分别说明普通、周期预期、已物化和受阻状态。
