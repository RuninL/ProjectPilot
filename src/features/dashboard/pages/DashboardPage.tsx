...
export function DashboardPage() {
    ...
    try {
      ...
      setData({
        ...
        upcoming: openTasks.filter(
          (task) =>
            task.due_date !== null && task.due_date > today && task.due_date <= addDays(today, 7),
        ),
        risks,
      });
    } catch (caught) {
  ...
  const highRisks = data.risks.filter(
    (risk) =>
      ['critical', 'high'].includes(risk.level) && ['open', 'monitoring'].includes(risk.status),
  );
  const cards = [
  ...
  const taskGroup = (title: string, tasks: TaskWithProject[], empty: string) => (
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {taskGroup('今日到期', data.today, '今天没有到期的未完成任务。')}
        {taskGroup('即将到期（7 天内）', data.upcoming, '未来 7 天没有到期的未完成任务。')}
        {taskGroup('已逾期', data.overdue, '没有已逾期的未完成任务。')}
      </div>
      <section className="rounded-lg border bg-card p-4">
        ...
        {data.projects.length === 0 ? (
          <EmptyState
            title="暂无进行中项目"
            description="创建项目后可在这里查看完成进度和任务健康度。"
          />
        ) : (
          <ul className="space-y-2">
            {data.projects.map((project) => {
              const tasks = [...data.today, ...data.upcoming, ...data.overdue].filter(
              ...
              return (
                <li key={project.id}>
                  ...
                  >
                    ...
                    <span>
                      {computeProjectProgress(tasks).percent}% · 待关注 {tasks.length}
                    </span>
                  </Link>
                </li>
              );
      ...
      <section className="rounded-lg border bg-card p-4">
        ...
        {data.risks
          .filter((risk) => ['open', 'monitoring'].includes(risk.status))
          .slice(0, 5)
          .map((risk) => (
            <Link
              key={risk.id}
              to={`/projects/${risk.project_id}#project-risks`}
              className="mb-2 flex justify-between text-sm hover:underline"
            >
              <span>
        ...
        {data.risks.length === 0 && <p className="text-sm text-muted-foreground">暂无开放风险。</p>}
      </section>
    </div>
  );
}
