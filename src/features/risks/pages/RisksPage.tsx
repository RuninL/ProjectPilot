...
export function RisksPage() {
  ...
  return (
    <div className="p-6">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            优先显示开放风险，按等级、截止日期和更新时间排序。
          </p>
        </div>
        <Button asChild>
          <Link to="/projects">在项目中新建风险</Link>
        </Button>
      </header>
      <Input
        aria-label="搜索风险"
      ...
      />
      {risks.length === 0 ? (
        <EmptyState title="暂无风险" description="在项目详情页登记风险后，会在这里集中跟踪。" />
      ) : (
        <ul className="divide-y rounded-lg border bg-card">
          {risks.map((risk) => (
            <li key={risk.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {levelLabel[risk.level]}风险
                  </Badge>
                  <Badge variant="outline">{statusLabel[risk.status]}</Badge>
                </div>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDelete === null
            ? ''
            : `确定删除「${pendingDelete.title}」（${pendingDelete.project_name}）吗？此操作不会删除项目、任务、会议、行动项或里程碑。`
        }
        confirmLabel="删除"
        destructive
        onCancel={() => {
