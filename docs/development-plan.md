...
| **3**              | 依赖 + Gantt（自研 SVG）                                               | task_dependencies 创建/删除 + 加边防环（migration 0003 触发器兜底同项目与直接反向边）；dependencyGraph.ts（构图/防环/Kahn/前驱后继/blocked 传导/排期冲突）；ganttViewModel + 纯 SVG renderer（周/月/季轴、时间条、依赖线、今日线、冲突高亮、图例；菱形留待阶段 4）                                                                    | Gantt 稳定正确，检测全通过单测                          |
